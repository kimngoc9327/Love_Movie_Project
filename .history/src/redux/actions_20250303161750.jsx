...
const AUTH_TOKEN = import.meta.API_TOKEN;

const getMovies = () => async (dispatch) => {
  try {
    const response = await axios.get(
      `${API_BASE_URL}/trending/movie/week?api_key=${import.meta.API_KEY}`,
      {
        headers: {
...
const getMovie = (id) => async (dispatch) => {
  try {
    const response = await axios.get(
      `${API_BASE_URL}/movie/${id}?append_to_response=videos,translations&api_key=${
        import.meta.API_KEY
      }`,
      {
        headers: {
